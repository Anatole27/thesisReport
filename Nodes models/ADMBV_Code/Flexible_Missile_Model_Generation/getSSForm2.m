function [flexibleBody,Vm] = getSSForm2(om,z,Bm,Cmz,Cmv,Do,n,Phi)
...
for i = 1:n_modes
    ynames{i+3*n-2} = strcat('z_{m',num2str(i),'}');
end
for i = 1:n
    ynames{i+3*n+n_modes-2} = strcat('z_{',num2str(i),'}');
end

%% Create SS system

flexibleBody = ss(Am,Bm,Cm,Dm);
flexibleBody.u = unames;
flexibleBody.y = ynames;
flexibleBody.StateName = xnames;